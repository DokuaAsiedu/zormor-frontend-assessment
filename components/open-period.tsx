...
export function OpenPeriod({periods, handlePeriods}: {periods: Period[], handlePeriods: (prev: Period[]) => void}) {
    ...
    <div>
      <div className="h-fit flex flex-row items-stretch flex-wrap gap-2 text-sm">
        {DAYS_OF_WEEK.map((item, index) => {
          return (
            <button type="button" key={`item-${index}`} className={`h-full p-1  grid place-items-center aspect-square capitalize  border-[1px] border-blue-400 rounded-full ${usedDays.includes(item) ? 'bg-blue-400 text-white' : 'bg-white text-blue-600'}`} onClick={() => addDay(item)}>
              {item}
            </button>
          )
        })}
      ...
      <>
        ...
        {usedDays.length < 7 && <GrAddCircle className="text-green-500" onClick={addPeriod}/>}
      </>
    </div>
  )
}
